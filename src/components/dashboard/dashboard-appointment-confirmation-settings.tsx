...
export function DashboardAppointmentConfirmationSettings({
  ...
  async function persist(next: boolean) {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/dashboard/appointment-confirmation", {
        method: "PATCH",
  ...
  const row = (
    ...
    >
      ...
      <span className="min-w-0 flex-1 text-[16px] font-extrabold leading-tight text-bakery-ink">
        {labels.appointmentConfirmationTitle}
      </span>
      <Toggle
        enabled={required}
        onChange={handleToggle}
        disabled={saving}
        ariaLabel={labels.appointmentConfirmationTitle}
      />
    </div>
  );
  ...
  const confirmSheet = (
    <DashboardActionSheet
      ...
      title={labels.appointmentConfirmationEnableConfirmTitle}
      ariaLabel={labels.appointmentConfirmationEnableConfirmTitle}
      placement="center"
      expanded={false}
      fitContent
    ...
    >
      <div className="space-y-6 px-2 py-2 text-center">
        <h2 className="text-[17px] font-extrabold text-bakery-ink">
          {labels.appointmentConfirmationEnableConfirmTitle}
        </h2>
        <p className="text-[15px] font-semibold leading-relaxed text-bakery-ink">
          {labels.appointmentConfirmationEnableConfirmBody}
        </p>
        ...
        <Wrapper>
          <DashboardHelpText>
            <p className="text-center text-[13px] font-semibold text-bakery-muted">
              {labels.appointmentConfirmationHint}
            </p>
          </DashboardHelpText>
        </Wrapper>
        {error ? (
